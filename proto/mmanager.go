...
func (mm *MManager) LoadAuthentication(phonenumber string, preferredAddr string) (*MConn, error) {
	...
	userFull, err := mconn.UsersGetFullUsers(inputUser)
	if err != nil {
...
func (mm *MManager) NewAuthentication(phonenumber string, addr string, useIPv6 bool) (*MConn, *TypeAuthSentCode, error) {
	// req connect
	respCh := make(chan sessionResponse)
